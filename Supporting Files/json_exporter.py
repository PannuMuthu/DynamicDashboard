...
class JsonCollector(object):
  ...
  def collect(self):
    ...
    metric.add_sample('abc_requests_duration_seconds_count',
        value=response['requests_handled'], labels={})
    metric.add_sample('abc_requests_duration_seconds_sum',
        value=response['requests_duration_milliseconds'] / 1000.0, labels={})
    yield metric

    # Counter for the failures
    metric = Metric('abc_requests_failed_total',
       'Requests failed', 'summary')
    metric.add_sample('abc_requests_failed_total',
       value=response['request_failures'], labels={})
    yield metric

    # Metrics with labels for the documents loaded
    metric = Metric('abc_documents_loaded', 'Requests failed', 'gauge')
    for k, v in response['documents_loaded'].items():
      metric.add_sample('abc_documents_loaded', value=v, labels={'repository': k})
    yield metric
